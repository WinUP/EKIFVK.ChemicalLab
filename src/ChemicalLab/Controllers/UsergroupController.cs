...
namespace EKIFVK.Todo.API.Controllers
{
    ...
    public class UsergroupController : UserBasedController
    {
        ...
        public UsergroupController(DatabaseContext database, IPermissionService checker, IOptions<SystemConsts> consts)
            : base(database, checker, consts) { }

        private SystemUsergroup FindGroup(string name)
        {
            return Database.SystemUsergroup.FirstOrDefault(e => e.Name == name);
        }
